...
async function findProjectRoot(startDir) {
	...
	while (true) {
		try {
			if (await fs.promises.stat(path.join(current, ".git"))) return current;
		} catch {}
...
function parseFrontmatter(content) {
	...
	try {
		...
		if (parsed && typeof parsed === "object") return {
			...
			whenToUse: typeof parsed.whenToUse === "string" ? parsed.whenToUse : void 0
		};
	} catch {}
	return {};
}
/** Scan one folder for skills (directories with SKILL.md or flat .md files) */
async function scanDirectory(dirPath, enabled, scope, source, parentDir) {
	const items = [];
	try {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				let whenToUse;
				try {
					const fm = parseFrontmatter(await fs.promises.readFile(skillMdPath, "utf8"));
				...
				let whenToUse;
				try {
					const fm = parseFrontmatter(await fs.promises.readFile(filePath, "utf8"));
					if (fm.name) name = fm.name;
					if (fm.description) description = fm.description.trim();
					if (fm.whenToUse) whenToUse = fm.whenToUse.trim();
				} catch {}
...
async function collectSkillsForParent(parentDir, scope, source) {
	...
	for (const item of disabledItems) if (!map.has(item.filename)) map.set(item.filename, item);
	for (const item of legacyDisabledItems) if (!map.has(item.filename)) map.set(item.filename, item);
	for (const item of nestedDisabledItems) if (!map.has(item.filename)) map.set(item.filename, item);
	return Array.from(map.values());
}
/** Read global MCP definitions from ~/.dsh/cordis.patch.yml */
async function readGlobalMcps(cordisPatchPath) {
	const mcps = [];
	try {
		const content = await fs.promises.readFile(cordisPatchPath, "utf8");
		const contents = (0, import_dist.parseDocument)(content).contents;
		if ((0, import_dist.isSeq)(contents)) {
			for (const item of contents.items) if ((0, import_dist.isMap)(item)) {
				const insertSeq = item.get("insert");
				if ((0, import_dist.isSeq)(insertSeq)) {
					for (const pluginItem of insertSeq.items) if ((0, import_dist.isMap)(pluginItem)) {
						const id = String(pluginItem.get("id") ?? "");
						const name = String(pluginItem.get("name") ?? "");
						const config = pluginItem.get("config");
						if (name === "@deepseek-ai/dsh-mcp-client" || id.startsWith("mcp-") || (0, import_dist.isMap)(config) && config.has("serverName")) {
							const serverName = (0, import_dist.isMap)(config) && String(config.get("serverName") ?? "") || id;
							const transport = (0, import_dist.isMap)(config) && String(config.get("transport") ?? "") || "stdio";
							const command = (0, import_dist.isMap)(config) && config.has("command") ? String(config.get("command")) : void 0;
							const url = (0, import_dist.isMap)(config) && config.has("url") ? String(config.get("url")) : void 0;
							let args;
							if ((0, import_dist.isMap)(config) && config.has("args")) {
								const rawArgs = config.get("args");
								if ((0, import_dist.isSeq)(rawArgs)) args = rawArgs.toJSON().map(String);
							}
							const rawDisabled = pluginItem.get("disabled");
...
async function toggleMcp(cordisPatchPath, payload) {
	const content = await fs.promises.readFile(cordisPatchPath, "utf8");
	const doc = (0, import_dist.parseDocument)(content);
	const contents = doc.contents;
	let modified = false;
	if ((0, import_dist.isSeq)(contents)) for (const item of contents.items) {
		if ((0, import_dist.isMap)(item)) {
			const insertSeq = item.get("insert");
			if ((0, import_dist.isSeq)(insertSeq)) {
				for (const pluginItem of insertSeq.items) if ((0, import_dist.isMap)(pluginItem)) {
					const id = String(pluginItem.get("id") ?? "");
					const config = pluginItem.get("config");
					const serverName = (0, import_dist.isMap)(config) ? String(config.get("serverName") ?? "") : "";
					if (id === payload.id || payload.serverName && serverName === payload.serverName) {
						if (payload.enabled) pluginItem.delete("disabled");
						else pluginItem.set("disabled", true);
						modified = true;
						break;
					}
				}
...
async function toggleSkill(payload) {
	...
	try {
		...
		if (err?.code === "EXDEV") {
			...
			await fs.promises.rm(payload.path, {
				recursive: true,
				force: true
			});
		} else throw err;
...
function apply(ctx) {
	...
	if (connection && connection.rpc) connection.rpc.handle(RPC_CHANNEL, async (endpoint, payload) => {
		try {
			if (endpoint === "list") {
				let cwd = process.cwd();
				...
				if (workspaceRegistry && typeof workspaceRegistry.list === "function") {
					...
					if (Array.isArray(workspaces)) {
						if (payload?.sessionId) {
							const ws = workspaces.find((w) => Array.isArray(w.sessionIds) && w.sessionIds.includes(payload.sessionId));
							if (ws?.path) cwd = ws.path;
						} else if (workspaces[0]?.path) cwd = workspaces[0].path;
					}
				}
				const projectRoot = await findProjectRoot(cwd);
				const [projectDshSkills, projectAgentsSkills] = await Promise.all([collectSkillsForParent(path.join(projectRoot, ".dsh"), "project", "project-dsh"), collectSkillsForParent(path.join(projectRoot, ".agents"), "project", "project-agents")]);
			...
			if (endpoint === "toggle-skill") return {
				ok: true,
				value: await toggleSkill(payload)
			};
			if (endpoint === "toggle-mcp") {
				await toggleMcp(cordisPatchPath, payload);
				return {
					ok: true,
